...
bool CanDriverHW::parseAndSetupJoints(const ros::NodeHandle &pnh)
{
    ...
    for (const auto &p : parsed) {
        ...
        if (jc.protocol == CanType::ECB) {
            if (std::fabs(jc.positionScale - 1.0) < 1e-12 ||
                std::fabs(jc.velocityScale - 1.0) < 1e-12) {
                ROS_WARN("[CanDriverHW] ECB joint '%s' still uses default scale=1.0. "
                         "Recommended: position_scale=6.283185307179586e-4, "
                         "velocity_scale=1.7453292519943296e-2.",
                         jc.name.c_str());
            }
        }
        ...
        if (jc.protocol == CanType::ECB) {
            auto baseProto = deviceManager_->getProtocol(jc.canDevice, jc.protocol);
            auto ecbProto = std::dynamic_pointer_cast<InnfosEcbProtocol>(baseProto);
            if (!ecbProto) {
...
void CanDriverHW::startMotorRefreshThreads()
{
    ...
    std::map<std::string, std::vector<MotorID>> ecbIds;
    for (const auto &jc : joints_) {
        ...
        } else if (jc.protocol == CanType::ECB) {
            ecbIds[jc.canDevice].push_back(jc.motorId);
        }
    }
    for (auto &kv : mtIds) {
    ...
    for (auto &kv : ecbIds) {
        deviceManager_->startRefresh(kv.first, CanType::ECB, kv.second);
    }
}
